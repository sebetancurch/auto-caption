...
def find_tool(name: str) -> str:
    ...
    candidates = [os.path.expandvars(rf"%LOCALAPPDATA%\Microsoft\WinGet\Links\{name}.exe")]
    ...
    for candidate in candidates:
        if os.path.isfile(candidate):
            return candidate
    raise FFmpegNotFound(f"{name} not found. Install it with:  winget install Gyan.FFmpeg")
